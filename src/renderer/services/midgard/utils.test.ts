...
describe('services/midgard/utils/', () => {
  ...
  describe('getPricePools', () => {
    ...
    const BUSDB: PoolDetail = { asset: 'BNB.BUSDB-000', assetDepth: '3', runeDepth: '33' }
    ...
    it('returns list of price pools in a right order', () => {
      const result = getPricePools([tomob, eth, BUSDB, btc, lok], PRICE_POOLS_WHITELIST)
      // RUNE pool
      ...
      expect(tusdPool.asset).toEqual(PoolAsset.BUSDB)
  ...
  describe('pricePoolSelector', () => {
    ...
    const BUSDB: PricePool = { asset: PoolAsset.BUSDB, poolData }
    const btc: PricePool = { asset: PoolAsset.BTC, poolData }

    it('selects ETH pool', () => {
      const pool = pricePoolSelector([RUNE_PRICE_POOL, eth, BUSDB, btc], O.some(PoolAsset.ETH))
      expect(pool.asset).toEqual(PoolAsset.ETH)
    })

    it('selects BUSDB pool if ETH pool is not available', () => {
      const pool = pricePoolSelector([RUNE_PRICE_POOL, BUSDB, btc], O.some(PoolAsset.ETH))
      expect(pool.asset).toEqual(PoolAsset.BUSDB)
    })

    it('selects BUSDB by default if no selection has been done', () => {
      const pool = pricePoolSelector([RUNE_PRICE_POOL, eth, BUSDB, btc], O.none)
      expect(pool.asset).toEqual(PoolAsset.BUSDB)
    })

    it('selects RUNE if ETH + BUSDB pools are not available', () => {
  ...
  describe('pricePoolSelectorFromRD', () => {
    ...
    it('selects ETH pool', () => {
      const poolsRD = mockPoolsStateSuccess([RUNE_PRICE_POOL, eth, BUSDB, btc])
      const pool = pricePoolSelectorFromRD(poolsRD, O.some(PoolAsset.ETH))
      expect(pool.asset).toEqual(PoolAsset.ETH)
    })

    it('selects BUSDB pool if ETH pool is not available', () => {
      const poolsRD = mockPoolsStateSuccess([RUNE_PRICE_POOL, BUSDB, btc])
      const pool = pricePoolSelectorFromRD(poolsRD, O.some(PoolAsset.ETH))
      expect(pool.asset).toEqual(PoolAsset.BUSDB)
    })
