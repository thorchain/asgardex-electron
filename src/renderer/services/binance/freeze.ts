...
const tx$ = ({
  ...
  clientState$.pipe(
    ...
    switchMap((client) => {
      // freeze
      if (action === 'freeze') {
        return Rx.from(client.freeze({ amount: amount.amount().toNumber(), asset: asset.symbol }))
      }
      // unfreeze
      if (action === 'unfreeze') {
        return Rx.from(client.unfreeze({ amount: amount.amount().toNumber(), asset: asset.symbol }))
      }
      return Rx.EMPTY
    }),
