...
const tx$ = ({
  ...
  clientState$.pipe(
    ...
    switchMap((client) =>
      memo
        ? Rx.from(client.vaultTx({ addressTo: to, amount: amount.amount().toNumber(), asset: symbol, memo }))
        : Rx.from(client.normalTx({ addressTo: to, amount: amount.amount().toNumber(), asset: symbol }))
    ),
