...
export interface GetPoolLegacyRequest {
    asset: string;
}
...
export class DefaultApi extends BaseAPI {
    ...
    /**
     * List actions along with their related transaction. An action is generated by one or more inbound transactions with the intended action set in the memo. The action may result in one or more outbound transactions. Results are paginated by sets of 50. Filters may be applied to query actions.
     * Get Actions
     */
     ...
     * Get details for a specific pool
     */
    getPoolLegacy = ({ asset }: GetPoolLegacyRequest): Observable<PoolLegacyDetail> => {
        throwIfNullOrUndefined(asset, 'getPoolLegacy');

        return this.request<PoolLegacyDetail>({
            path: '/v2/pool-legacy/{asset}'.replace('{asset}', encodeURI(asset)),
            method: 'GET',
        });
    };
