...
import { WALLET } from '../../../contexts/WalletDatastore/DataStoreContext'
...
const ImportMnemonicForm: React.FC = (): JSX.Element => {
  ...
  const importMnemonicWallet = async (mnemonic: string, password: string) => {
    if (!localStorage.getItem('keystore')) {
      try {
        await WALLET.generateNewWallet(password, mnemonic, null, null)
        console.info("SUCCESS ON NEW WALLET")
      } catch (error) {
        console.error(error)
      }
      ...
      // const UserBalances = new BinanceBalancesService()
      ...
      //     await UserBalances.insert(balances)
      //   }
      // }).catch((e:any) => {
      //   console.log(e)
      // })
